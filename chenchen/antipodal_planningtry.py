...
import chenchen.animation_finger_smallitems as pm
...
if __name__ == '__main__':
    ...
    while count < samples_number:
        ...
        get_figure(gripper=gripper_s, target_pos=obj_target_pos, rot_list=obj_rot_list, rgba=[0, 0.3, 0, 0.1],
                   gripper_pos_list=gripper_pos_list, gripper_rot_list=gripper_rot_list)
        count = count + 1
    rbt_s = gf5.GOFA5()
    ...
    gripper_pos3 = gripper_pos2 + [0, 0, 0.2]
    ...
    goal_jnt_values5 = rbt_s.ik(tgt_pos=final_pos, tgt_rotmat=final_rot)
    ...
    arm_len4 = len(arm_path)
    arm_path = arm_path + arm_path5
    arm_len5 = len(arm_path)
    ...
    rgt_path4 = pm.rgt_path
    lft_path4 = pm.lft_path
    rgt_path3 = np.linspace([math.pi / 4, math.pi / 4, 0], rgt_path4[0], 300, endpoint=False)
    rgt_path3 = [np.array(sublist) for sublist in rgt_path3]
    lft_path3 = np.linspace([math.pi / 4, 0, 0], lft_path4[0], 300, endpoint=False)
    lft_path3 = [np.array(sublist) for sublist in lft_path3]
    lft_path = []
    lft_path = lft_path + lft_path1
    lft_path = lft_path + lft_path2
    lft_path = lft_path + lft_path3
    lft_path = lft_path + lft_path4
    rgt_path = []
    rgt_path = rgt_path + rgt_path1
    rgt_path = rgt_path + rgt_path2
    rgt_path = rgt_path + rgt_path3
    rgt_path = rgt_path + rgt_path4
    rbt_s_mesh = []
    armcounter = [0]
    flag = [0]
    handcounter = [0]
        ...
        if handcounter[0] == 0 or handcounter[0] >= lft_len1 - 1:
            ...
            if flag[0] == 0:
                ...
                if armcounter[0] == len(path) - 1:
                    flag[0] = 1
        if armcounter[0] > arm_len1 - 1 and handcounter[0] <= lft_len1 - 1 or armcounter[0] >= arm_len3 - 1 and \
                armcounter[0] <= arm_len4 - 1 or armcounter[0] >= arm_len5 - 1:
