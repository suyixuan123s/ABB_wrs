'''
Logger GoFa motions for debug purposes.
Author: Jacky Liang
'''
...
class GoFaMotionLogger:
    ...
    def flush_to_file(self, filename):
        if not filename.endswith(GoFaMotionLogger._SUFFIX):
            raise Exception("Can only flush to {0} files!".format(GoFaMotionLogger._SUFFIX))
    ...
    def load(filename):
        if not filename.endswith(GoFaMotionLogger._SUFFIX):
            raise Exception("Can only load from {0} files!".format(GoFaMotionLogger._SUFFIX))
