...
class RigidTransform(object):
    ...
    def __init__(self,
        ...
        translation : :obj:`None` or :obj:`numpy.ndarray` of int
    ...
    def _check_valid_ext_axis(self, ext_axis: float):
        """ Checks that the ext_axis is valid."""
        if not isinstance(ext_axis, float) or not np.issubdtype(type(ext_axis), np.number):
            raise ValueError('Ext_axis must be specified as numeric float')
    ...
    @property
    def ext_axis(self):
        return self._ext_axis

    @ext_axis.setter
    def ext_axis(self, ext_axis):
        self._check_valid_ext_axis(ext_axis)
        self._ext_axis = ext_axis
        ...
        if points.frame != self._from_frame:
            raise ValueError(
                'Cannot transform points in frame %s with rigid transformation from frame %s to frame %s' % (
                    points.frame, self._from_frame, self._to_frame))
    ...
    def dot(self, other_tf):
        ...
        if other_tf.to_frame != self.from_frame:
            raise ValueError('To frame of right hand side ({0}) must match from frame of left hand side ({1})'.format(
                other_tf.to_frame, self.from_frame))
    ...
    def save(self, filename):
        ...
        if file_ext.lower() != TF_EXTENSION:
            raise ValueError('Extension %s not supported for RigidTransform. Must be stored with extension %s' % (
                file_ext, TF_EXTENSION))
    ...
    def publish_to_ros(self, mode='transform', service_name='rigid_transforms/rigid_transform_publisher',
                       namespace=None):
    ...
    def interpolate(T0, T1, t):
        ...
        if T0.to_frame != T1.to_frame:
            raise ValueError(
                'Cannot interpolate between 2 transforms with different to frames! Got T1 {0} and T2 {1}'.format(
                    T0.to_frame, T1.to_frame))
    ...
    def load(filename):
        ...
        if file_ext.lower() != TF_EXTENSION:
            raise ValueError(
                'Extension %s not supported for RigidTransform. Can only load extension %s' % (file_ext, TF_EXTENSION))
...
class SimilarityTransform(RigidTransform):
    ...
    def save(self, filename):
        ...
        if file_ext.lower() != STF_EXTENSION:
            raise ValueError('Extension %s not supported for SimilarityTransform. Must be stored with extension %s' % (
                file_ext, STF_EXTENSION))
    ...
    def load(filename):
        ...
        file_root, file_ext = os.path.splitext(filename)
        if file_ext.lower() != STF_EXTENSION:
            raise ValueError('Extension %s not supported for SimilarityTransform. Can only load extension %s' % (
                file_ext, STF_EXTENSION))
