...
CONTROL_TABLE = {
    'X_SERIES': DXL_CONTROL_TABLE(ADDR_OPERATION_MODE=11,
                                  ...
                                  DXL_MIN_PRO_VEL_VAL=0,
                                  DXL_MAX_PRO_VEL_VAL=1000,
                                  )
}
...
class DynamixelMotor(object):
    ...
    def set_dxl_pro_vel(self, vel, dxl_id: int = 1) -> bool:
        """
        ...
        assert self._control_table.DXL_MIN_PRO_VEL_VAL <= vel <= self._control_table.DXL_MAX_PRO_VEL_VAL
        dxl_comm_result, dxl_error = self._packet_handler.write4ByteTxRx(port=self._port_handler,
                                                                         dxl_id=dxl_id,
                                                                         address=self._control_table.ADDR_PRO_VEL,
                                                                         data=vel)
        if dxl_comm_result != COMM_SUCCESS:
            # print("%s" % self._packet_handler.getTxRxResult(dxl_comm_result))
            return False
