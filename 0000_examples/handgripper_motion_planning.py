...
if __name__ == '__main__':
    ...
    def get_path(start_joint,end_joint,moveinterval):
        ...
        for path in stepwisepath:
            goal_jnt_values = robot_s.ik(tgt_pos=path, tgt_rotmat=rot_end, seed_jnt_values=goal_jnt_values,component_name="rgtfinger")
    ...
    path = get_path(start_joint=np.array([math.pi * 1/ 4 , math.pi * 1 / 4 , -math.pi * 0 / 2]),end_joint=np.array([0 ,0 , math.pi * 1 / 2]),moveinterval=10)
    print(path)
    base.run()
