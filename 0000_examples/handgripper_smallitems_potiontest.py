...
if __name__ == '__main__':
    ...
    def get_path(start_joint,end_joint,moveinterval):
        ...
        for path in stepwisepath:
            goal_jnt_values = robot_s.ik(tgt_pos=path, tgt_rotmat=rot_end, seed_jnt_values=goal_jnt_values,component_name="lftfinger")
    ...
    path = get_path(start_joint=np.array([math.pi * 4/ 15 , math.pi * 4 / 15 , math.pi * 4 / 15]),end_joint=np.array([math.pi * 0/ 4 , math.pi * 0 / 4 , math.pi * 4 / 5]),moveinterval=100)
    print(path)
    base.run()
