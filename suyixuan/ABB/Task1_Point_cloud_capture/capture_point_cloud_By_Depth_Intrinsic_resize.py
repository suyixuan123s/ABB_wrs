...
import cv2
from suyixuan.ABB.Task2_Finding_transformation_matrix.transform_to_simulation import transform_to_simulation
...
try:
    while True:
        ...
        if key == 13:
            ...
            # **使用transform_to_simulation函数转换点云到仿真坐标系**
            transformed_point_cloud = transform_to_simulation(point_cloud)

            # 裁剪点云，只保留仿真平台桌面范围内的点
            x_range = (-0.14, 1.06)  # x 轴范围
            y_range = (-0.71, 0.80)  # y 轴范围
            z_range = (-0.015, 0.60)  # z 轴范围
            cropped_point_cloud = crop_point_cloud(transformed_point_cloud, x_range, y_range, z_range)

            save_path = r'/suyixuan/ABB/ABB-11-6/Dataset/colored_point_cloud1118.ply'
