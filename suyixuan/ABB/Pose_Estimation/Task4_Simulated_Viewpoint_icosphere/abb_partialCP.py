...
# 使用tab20 colormap生成颜色
def generate_colormap_color(index, total_colors=20):
    # cmap = plt.cm.get_cmap('tab20', total_colors)  # 获取tab20 colormap
    cmap = plt.colormaps['tab20']
    return cmap(index % total_colors)  # 获取对应的颜色
...
if __name__ == '__main__':
    ...
    # # 为icosphere的每个顶点生成不同颜色的小球
    # for i, pnt in enumerate(sample):
    #     color = generate_colormap_color(i)  # 获取顶点对应的颜色
    #     gm.gen_sphere(pnt, 0.03, color[:4]).attach_to(base)  # 使用颜色的前4个值（RGBA）
    ...
    viewedmesh_o3d = o3d.io.read_triangle_mesh("../Task3_ICP_pre-processing/temp.stl")
    viewedmesh_o3d.compute_vertex_normals()
    pcd = viewedmesh_o3d.sample_points_poisson_disk(number_of_points=40000)
