...
# 设置输出目录并确保目录存在
output_dir = r"E:\ABB-Project\ABB_wrs\suyixuan\ABB\Pose_Estimation\Task6_Point_cloud_processing\Dataset_Point_Cloud"
...
# 设置文件路径
stl_file_path = r"E:\ABB-Project\ABB_wrs\suyixuan\ABB\Pose_Estimation\Task3_ICP_pre-processing\temp.stl"  # 长方体的 STL 文件路径
realsense_ply_file = r"E:\ABB-Project\ABB_wrs\suyixuan\ABB\Pose_Estimation\Task3_ICP_pre-processing\TexturedPointCloud.ply"  # Realsense 点云文件路径

# 生成 STL 文件对应的点云
stl_point_cloud = stl_to_point_cloud(stl_file_path, 40000)
...
# 保存归一化后的 STL 点云
stl_output_file = os.path.join(output_dir, "normalized_stl_point_cloud.ply")
...
# 保存归一化后的 Realsense 点云
realsense_output_file = os.path.join(output_dir, "normalized_realsense_point_cloud.ply")
if o3d.io.write_point_cloud(realsense_output_file, realsense_point_cloud_normalized):
    print(f"Realsense 点云成功保存到: {realsense_output_file}")
else:
    print(f"保存 Realsense 点云失败: {realsense_output_file}")
