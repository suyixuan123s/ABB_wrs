...
if __name__ == '__main__':
    ...
    icosphere = gm.gen_sphere(radius=30, rgba=[0, 0, 1, 0.1], subdivisions=0)
    sample = icosphere.objtrm.vertices
    for pnt in sample:
        gm.gen_sphere(pnt, 0.3, [0, 1, 0, 1]).attach_to(base)
    icosphere.set_rgba([0, 1, 1, 0.1])
    icosphere.attach_to(base)

    # base.run()
    # name = "mug.stl"
    name = "rack_5ml_new1.stl"
    # name = "airplaneremesh"
    # name = "CoughDropsBerries_800_tex.obj"
    # name = "armadillo.stl"
    # name = "mug"
    # obj_gm = gm.GeometricModel(f"E:\ABB-Project\ABB_wrs\suyixuan\ABB\Pose_Estimation\Task5_ICP_GOFA5\meshes/{name}")
    # obj_gm.set_rgba([0, 1, 0, 0.1])
    # obj_gm.attach_to(base)


    obj = tw.TrimeshHu(r"E:\ABB-Project\ABB_wrs\suyixuan\ABB\Pose_Estimation\Task5_ICP_GOFA5\meshes", name, scale=0.001)
    ...
    gm.gen_sphere(sample[5], 3).attach_to(base)
