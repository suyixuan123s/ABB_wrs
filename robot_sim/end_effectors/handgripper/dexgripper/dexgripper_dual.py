...
class UR3EDual(gf.DexGripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='ur3edual', enable_cc=True):
        ...
        self.lft_arm = lf.Leftfinger(pos=self.lft_body.jnts[0]['gl_posq'],
                            rotmat=self.lft_body.jnts[0]['gl_rotmatq'],
                            homeconf=np.zeros(3),
        ...
        self.rgt_arm = lf.Leftfinger(pos=self.lft_body.jnts[0]['gl_posq'],
                                        rotmat=np.dot(rm.rotmat_from_axangle(np.array([0,0,1]), np.deg2rad(90)),self.lft_body.jnts[-1]['gl_rotmatq']),
                                        homeconf=np.zeros(3),
                                        name='arm', enable_cc=False)
        # tool center point
        # lft
    ...
    def enable_cc(self):
        ...
        self.cc.add_cdlnks(self.lft_arm, [0, 1, 2, 3])
        self.cc.add_cdlnks(self.rgt_arm, [0, 1, 2, 3])
