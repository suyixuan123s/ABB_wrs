...
import robot_sim.end_effectors.handgripper.finger.finger_silica.fingersilica as fs
import random
...
class Handgripper(gf.DexGripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), name='hand_griper', enable_cc=True):
        ...
        # base
        self.base1 = jl.JLChain(pos=pos, rotmat=rotmat, homeconf=np.zeros(3), name='base1')
        self.base1.jnts[1]['loc_pos'] = np.array([0, 0.03919, 0.09419])
        self.base1.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(math.pi *3/4 , math.pi , math.pi)
        self.base2 = jl.JLChain(pos=pos, rotmat=rotmat, homeconf=np.zeros(3), name='base2')
        self.base2.jnts[1]['loc_pos'] = np.array([0, -0.03919, 0.09419])
        self.base2.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(math.pi *3/4 , math.pi , math.pi*0)
        self.base1.lnks[0]['name'] = "hand_gripper_base"
        self.base1.lnks[0]['loc_pos'] = np.array([0, 0, 0])
        self.base1.lnks[0]['loc_rotmat'] = rm.rotmat_from_euler(math.pi / 2.0, 0,
                                                                   math.pi / 2.0)  # left from robot_s view
        self.base1.lnks[0]['collision_model'] = cm.CollisionModel(
            os.path.join(this_dir, "meshes", "hand.stl"),
            cdprimit_type="box", expand_radius=.005)
        self.base1.lnks[0]['rgba'] = [.3, .3, .3, 1.0]
        # self.base.lnks[1]['name'] = "lft"

        # self.base.jnts[2]["name"] = "lft_base"


        self.base1.reinitialize()
        self.base2.reinitialize()

        self.base1.jnts[1]['loc_pos'] = np.array([0, 0.03919, 0.09419])

        self.rgtfinger = fs.Fingersilica(pos=self.base1.jnts[1]['gl_posq'],
                            rotmat=self.base1.jnts[1]['gl_rotmatq'],
                            homeconf=np.zeros(3),
                            name='rgtfinger', enable_cc=False)
        self.lftfinger = fn.Fingernails(pos=self.base2.jnts[1]['gl_posq'],
                            rotmat=self.base2.jnts[2]['gl_rotmatq'],
                            homeconf=np.zeros(3),
    ...
    def enable_cc(self):
        ...
        self.cc.add_cdlnks(self.base1, [0])
    ...
    def fix_to(self, pos, rotmat):
        self.pos = pos
        self.rotmat = rotmat
        self.base1.fix_to(self.pos, self.rotmat)
        self.base2.fix_to(self.pos, self.rotmat)
        lft_pos = self.base2.jnts[1]['gl_posq']
        lft_rotmat = self.base2.jnts[1]['gl_rotmatq']
        rgt_pos = self.base1.jnts[1]['gl_posq']
        rgt_rotmat = self.base1.jnts[1]['gl_rotmatq']
        self.lftfinger.fix_to(pos=lft_pos, rotmat=lft_rotmat)
        ...
        self.rgtfinger.fix_to(pos=rgt_pos, rotmat=rgt_rotmat)
    ...
    def gen_meshmodel(self,
        ...
        self.base1.gen_meshmodel(tcp_loc_pos=None,
                                    tcp_loc_rotmat=None,
                                    toggle_tcpcs=False,
                                    toggle_jntscs=toggle_jntscs,
    ...
    def preparation_grabbing(self):
        rgt_goal_jnt_values = np.array([math.pi * 1 / 5, -math.pi * 1 / 12 , -math.pi * 1 / 12])
        self.fk("rgtfinger", jnt_values=rgt_goal_jnt_values)
        lft_goal_jnt_values = np.array([math.pi * 1 / 4, math.pi * 1 / 2 , math.pi * 1 / 4])
        self.fk("lftfinger", jnt_values=lft_goal_jnt_values)

    def dif_grabbing1(self):
        rgt_goal_jnt_values = np.array([math.pi * 1 / 5, -math.pi * 1 / 6 , -math.pi * 1 / 6])
        self.fk("rgtfinger", jnt_values=rgt_goal_jnt_values)
        lft_goal_jnt_values = np.array([math.pi * 1 / 4, math.pi * 1 / 2 , math.pi * 1 / 4])
        self.fk("lftfinger", jnt_values=lft_goal_jnt_values)

    def random_grab(self):
        rgt_goal_jnt_values = np.array([random.random()*math.pi * 1 / 4, random.random()*-math.pi * 1 / 6 , random.random()*-math.pi * 1 / 6])
        self.fk("rgtfinger", jnt_values=rgt_goal_jnt_values)
        lft_goal_jnt_values = np.array([math.pi * 1 / 4, math.pi * 1 / 2 , math.pi * 1 / 4])
        self.fk("lftfinger", jnt_values=lft_goal_jnt_values)

    def dif_grabbing2(self):
        rgt_goal_jnt_values = np.array([math.pi * 1 / 5, -math.pi * 1 / 8 , -math.pi * 1 / 8])
        self.fk("rgtfinger", jnt_values=rgt_goal_jnt_values)
        lft_goal_jnt_values = np.array([math.pi * 1 / 4, math.pi * 1 / 2 , math.pi * 1 / 4])
        self.fk("lftfinger", jnt_values=lft_goal_jnt_values)

    def ssss(self):
        self.fk("both_arm", jnt_values=np.array([0.13757838,0.73808158,0.48650878,0.13757838,0.73808158,0.48650878]))
...
if __name__ == '__main__':
    ...
    hand_gripper.gen_meshmodel(rgba_rgtfinger=(1, 0, 0, 1), rgba_lftfinger=(1, 0, 0, 1), toggle_tcpcs=False,
                       toggle_jntscs=False).attach_to(base)

    print(hand_gripper.get_gl_tcp('rgtfinger'))
    base.run()
