...
class Fingersilica(fi.FingerInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(3), name='ur5e', enable_cc=True):
        ...
        self.jlc.jnts[4]['loc_pos'] = np.array([0, -0.00361, 0.03473])
        self.jlc.jnts[4]['loc_motionax'] = np.array([0, 0, 0])
        self.jlc.jnts[4]['loc_rotmat'] = rm.rotmat_from_euler(60/180*math.pi, 0, math.pi*0 )
        ...
        self.jlc.lnks[3]['mesh_file'] = os.path.join(this_dir, "meshes", "LINK04.STL")
...
if __name__ == '__main__':
    ...
    manipulator_instance = Fingersilica(enable_cc=True)
