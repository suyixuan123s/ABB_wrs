...
if __name__ == '__main__':
    ...
    width_detect = 0.005
    length_detect = 0.09
    gm.gen_cone(anchor - anchor_normal * length_detect, anchor, radius=length_detect * np.sin(np.pi * 20 / 180),
                sections=7, rgba=(0, 1, 0, 0.3)).attach_to(base)
    detect_ray = gm.gen_stick(anchor - anchor_normal * length_detect, anchor - anchor_normal * 0.01 * length_detect,
                              thickness=width_detect * np.sin(np.pi * 20 / 180) * 2, sections=7, rgba=(0, 1, 0, 0.3))
    ...
    gm.gen_arrow(anchor_opposite, anchor_opposite + anchor_opposite_normal * 0.015, rgba=(0, 0, 1, 1),
