...
class ContactDetector():
    ...
    def pairing(self):
        ...
        for i in self.vertex_id_clustered[self.corner_sampled_id]:
            ...
            cc_pnts, cc_pnts_id, cc_normal = self.find_antipodal(corner_detector, anchor, i, anchor_normal,
                                                                 self.corner_sampled,
                                                                 self.corner_sampled_id, self.corner_normal_sampled)
            ce_pnts, ce_pnts_id, ce_normal = self.find_antipodal(corner_detector, anchor, i, anchor_normal,
                                                                 self.edge_sampled,
                                                                 self.edge_sampled_id, self.edge_normal_sampled,
                                                                 threshold=0.9)
            cf_pnts, cf_pnts_id, cf_normal = self.find_antipodal(corner_detector, anchor, i, anchor_normal,
                                                                 self.surface_sampled,
                                                                 self.surface_sampled_id, self.surface_normal_sampled,
                                                                 threshold=0.9)
        ...
        for i in self.edge_id[self.edge_sampled_id][8:]:
            ...
            # gm.gen_arrow(anchor, anchor + 0.1 * rm.unit_vector(anchor_normal)).attach_to(base)
            ee_pnts, ee_pnts_id, ee_normal = self.find_antipodal(edge_detector, anchor, i, anchor_normal,
                                                                 self.edge_sampled,
                                                                 self.edge_sampled_id, self.edge_normal_sampled,
                                                                 threshold=0.95, debug=False)
            ef_pnts, ef_pnts_id, ef_normal = self.find_antipodal(edge_detector, anchor, i, anchor_normal,
                                                                 self.surface_sampled,
                                                                 self.surface_sampled_id, self.surface_normal_sampled,
                                                                 threshold=0.95, debug=False)
        ...
        for i in self.surface_id[self.surface_sampled_id][4:]:
            ...
            ff_pnts, ff_pnts_id, ff_normal = self.find_antipodal(surface_detector, anchor, i, anchor_normal,
                                                                 self.surface_sampled,
                                                                 self.surface_sampled_id, self.surface_normal_sampled,
                                                                 threshold=0.95, debug=False)
    ...
    def input_data_debug(self):
        for i, pnt in enumerate(self.corner_sampled):
            gm.gen_sphere(pnt, radius=0.003, rgba=[1, 0, 0, 1]).attach_to(base)
            gm.gen_sphere(self.pcd_np[self.vertex_id_clustered[self.corner_sampled_id[i]]], radius=0.01,
                          rgba=[1, 0, 0, 0.5]).attach_to(base)
        ...
        for i, pnt in enumerate(self.edge_sampled):
            ...
            gm.gen_sphere(self.pcd_np[self.edge_id[self.edge_sampled_id[i]]], radius=0.01,
        ...
        for i, pnt in enumerate(self.surface_sampled):
            ...
            gm.gen_sphere(self.pcd_np[self.surface_id[self.surface_sampled_id[i]]], radius=0.01,
    ...
    def get_corner_detector(self, corner_id=None, debug=False):
        ...
        detector = gm.gen_cone(spos=corner_pnt + self.detect_scale * edge_normal_uniform, epos=corner_pnt,
                               rgba=[1, 0, 0, 0.3], radius=self.detect_scale, sections=24)
