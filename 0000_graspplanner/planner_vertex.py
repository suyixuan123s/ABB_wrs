...
class GraspPlanner():
    ...
    def detect_edge(self, cloud_np, threshold_u=50, threshold_l=0.02, r=0.003, show_neighber=False, load=False,
                    show=True):
        r = r
        if load:
        ...
        for pair in combinations:
            ...
            if np.linalg.norm(pair[0] - pair[1], ord=2) > 0.085:
                ...
                if np.linalg.norm(pair[0] - pair[1], ord=2) > 0.045:
                    gripper.grip_at_with_jcpose(center, gl_jaw_center_rotmat=t_ar,
                                                jaw_width=np.linalg.norm(pair[0] - pair[1], ord=2))
                    ...
                    if gripper.is_mesh_collided(objcm_list=[self.collision_model]):
                        ...
                        for i, pnt in enumerate(contact_a_neighber_np):
                            ...
                            if abs(np.dot(pair[0] - pnt, rm.unit_vector(pair[1] - pair[0]))) < 0.001 and np.dot(
                                    rm.unit_vector(pair[1] - pair[0]), contact_a_normal_np[i]) < 0:
                        ...
                        for i, pnt in enumerate(contact_b_neighber_np):
                            ...
                            if abs(np.dot(pair[1] - pnt, rm.unit_vector(pair[0] - pair[1]))) < 0.001 and np.dot(
                                    rm.unit_vector(pair[0] - pair[1]), contact_b_normal_np[i]) < 0:
                                range_ball_cm.set_rgba((1, 0.3, 0.6, 1))
                            else:
                                range_ball_cm.set_rgba((1, 0.3, 0.6, 0.05))
                            range_ball_cm.attach_to(base)
    ...
    def _tune_sample(self, pcd_neighber_np, pcd_neighber_normal_np, threshold_u=50, threshold_l=0.02,
                     searchmaxPCA=False):
        datamean = pcd_neighber_np.mean(axis=0)
        eigen_vals, eigen_vecs = self.pca(pcd_neighber_np - datamean)
