    ...
    while p_cnt < len(p_narray):
        for stroke_len in stroke_len_list:
            ...
            while i < stroke_len:
                ...
                i += 1
                p_cnt += 1
            drawpath_resized.append(stroke)

    return drawpath_resized
...
def rayhitmesh_drawpath(obj_item, drawpath, direction=np.asarray((0, 0, 1)), error_method='ED', toggledebug=False):
    def __get_mean(l):
    ...
    for st in drawpath:
            ...
            try:
                ...
                if toggledebug:
                    if i == 1:
                        ...
                        ax.scatter([pos_nrml_list[0][0][0]], [pos_nrml_list[0][0][1]], [pos_nrml_list[0][0][2]], c='r',
                                   s=10)
                        ax.scatter([pos_nrml_list[1][0][0]], [pos_nrml_list[1][0][1]], [pos_nrml_list[1][0][2]], c='g',
                                   s=10)
                        plt.show()
            except:
        ...
        if error_method == 'GD':
            error, error_list = get_prj_error(drawpath, pos_nrml_list, method=error_method, pcd=obj_item.pcd)
        else:
...
def __find_nxt_p_pca(drawpath_p1, drawpath_p2, kdt_d3, p0, n0, max_nn=200, direction=np.array([0, 0, 1]),
...
def __find_nxt_p_psfc(drawpath_p1, drawpath_p2, kdt_d3, p0, n0, max_nn=150, direction=np.array([0, 0, 1]),
    ...
    def __surface(pts, mode):
        if mode == 'rbf':
            ...
            surface = sfc.BiBSpline(pts[:, :2], pts[:, 2])
        else:
            surface = None
        return surface
...
def __prj_stroke(stroke, drawcenter, pcd, pcd_nrmls, kdt_d3, mode='DI', objcm=None, pcd_start_p=None, error_method='ED',
                 pcd_start_n=None, surface=None, transmat=None, snap=False, direction=np.asarray((0, 0, 1)),
                 toggledebug=False):
    """
    ...
    :param pcd_nrmls:
    :param kdt_d3:
    :param mode: 'DI', 'EI','QI','rbf', 'gaussian', 'quad'，'rbf_g', 'bp'
    :param objcm:
    :param pcd_start_p:
    :param pcd_start_n:
    :param direction:
    ...
    for i in range(len(stroke) - 1):
        ...
        if mode == 'EI':
            ...
            print("mode name must in ['DI', 'EI', 'QI', 'rbf', 'rbf_g', 'gaussian', 'quad', 'bs', 'bp']")
...
def prj_drawpath(obj_item, drawpath, mode='DI', step=1.0, direction=np.asarray((0, 0, 1)), error_method='ED',
                 toggledebug=False, pca_trans=True, snap=False):
    if type(drawpath[0]) != type([]):
        drawpath = [drawpath]
    print(f'--------------map multiple strokes on pcd({mode})--------------')
    print('pcd num:', len(obj_item.pcd))
    print('stroke num:', len(drawpath))
    ...
    for i, stroke in enumerate(drawpath):
        ...
        if i > 0:
            spos = drawpath[i - 1][-1]
            gotostart_stroke = mu.linear_interp_2d(spos, stroke[0], step=step)
            gotostart_pos_nrml_list, _, _, time_cost = \
                __prj_stroke(gotostart_stroke, obj_item.drawcenter, obj_item.pcd, obj_item.nrmls, kdt_d3,
                             ...
                             toggledebug=toggledebug, surface=surface, transmat=transmat, snap=snap)
...
def prj_drawpath_SI(obj_item, drawpath, toggledebug=False):
    if type(drawpath[0]) != type([]):
        drawpath = [drawpath]
    time_cost_total = 0
    ...
    pos_nrml_list_ms = []
    for st in drawpath:
        pos_nrml_list, error, error_list, time_cost = __prj_stroke_SI(st, uv_center, uvs, vs, nrmls, faces, scale_list)
        pos_nrml_list_ms.append(pos_nrml_list)
        error_ms.append(error)
        error_list_ms.extend(error_list)
        time_cost_total += time_cost
...
def prj_drawpath_II(obj_item, drawpath, toggledebug=False):
    if type(drawpath[0]) != type([]):
        drawpath = [drawpath]
    ...
    for st in drawpath:
        pos_nrml_list, error, error_list = \
            __prj_stroke_II(st, uv_center, vs, nrmls, kdt_uv, scale_list, use_binp=True)
        pos_nrml_list_ms.append(pos_nrml_list)
        error_ms.append(error)
        error_list_ms.extend(error_list)
    print('avg error', np.mean(error_ms))
...
def prj_drawpat_II_temp(obj_item, drawpath, sample_num=None, toggledebug=False):
    if type(drawpath[0]) != type([]):
        drawpath = [drawpath]
    time_start = time.time()
...
def show_drawpath(pos_nrml_list, color=(1, 0, 0), show_nrmls=False, transparency=1.0):
    if type(pos_nrml_list[0][0]) == type([]):
        pos_nrml_list = flatten_nested_list(pos_nrml_list)
    ...
    def __cal_dist(stroke_pos_list, step=step):
        ...
        for i in range(0, x_strip + 1):
            for j in range(x_strip + 1, len(stroke_pos_list)):
                ...
                h_pre = h_id - 1 if h_id > 0 else 0
                h_pos = h_id + 1 if h_id < len(stroke_pos_list[i]) - 1 else len(stroke_pos_list[i]) - 1
                v_pre = v_id - 1 if v_id > 0 else 0
                v_pos = v_id + 1 if v_id < len(stroke_pos_list[j]) - 1 else len(stroke_pos_list[j]) - 1
                try:
                    p1 = np.asarray(stroke_pos_list[i][h_id])
                    p2 = np.asarray(stroke_pos_list[j][v_id])
                    l1 = np.asarray(stroke_pos_list[i][h_pos]) - np.asarray(stroke_pos_list[i][h_pre])
                    l2 = np.asarray(stroke_pos_list[j][v_pos]) - np.asarray(stroke_pos_list[j][v_pre])
                    angle = np.degrees(np.arccos(np.dot(l1, l2) / (np.linalg.norm(l1) * np.linalg.norm(l2))))
                except:
                    angle = 0
                    p1 = np.asarray((0, 0, 0))
                    p2 = np.asarray((0, 0, 0))
        ...
        return grid_points_dist, grid_angle_div

    stroke_pos_list = __extract_pos(pos_nrml_list)
    grid_points_dist, grid_angle_div = __cal_dist(stroke_pos_list)
    return len(grid_points_dist), round(np.average(grid_points_dist), 5), round(np.average(grid_angle_div), 5)
...
if __name__ == '__main__':
    ...
    # drawpath_ms = resize_drawpath(drawpath_ms, DRAWREC_SIZE[0], DRAWREC_SIZE[1], space=0)
    ...
    if stl_f_name is None:
        ...
        tgt_item = ru.get_obj_by_range(phxilocator, phoxi_f_name=phoxi_f_path, load=True, reconstruct_surface=True,
    ...
    """
    # # ray hit mesh
    # pos_nrml_list, error_list, time_cost = rayhitmesh_drawpath(tgt_item, drawpath, toggledebug=False)
    # show_drawpath(pos_nrml_list, color=(0, 1, 0))
    # print('rayhit time cost', time_cost)
    #
    # # conformal mapping
    # pos_nrml_list, error_list_cm, time_cost = prj_drawpath_SI(tgt_item, drawpath)
    # show_drawpath(pos_nrml_list, color=(0, 1, 0))
    #
    # # metrology based method
    # pos_nrml_list, error_list_msd, time_cost = \
    #     prj_drawpath(tgt_item, drawpath, mode='EI', direction=direction, toggledebug=False)
    # show_drawpath(pos_nrml_list, color=(0, 0, 1), show_nrmls=True)
    # base.run()
    #
    # # rbf
    # pos_nrml_list, error_list_rbf, time_cost = \
    #     prj_drawpath(tgt_item, drawpath, mode='rbf', direction=direction, toggledebug=False)
    # show_drawpath(pos_nrml_list, color=(0, 1, 0))
    # base.run()
    ...
    #     rayhitmesh_drawpath(tgt_item, drawpath_ms, error_method=error_method)
    ...
    #    prj_drawpath_SI(tgt_item, drawpath_ms, toggledebug=True)
    ...
    #
    # pos_nrml_list, error_list_cm, time_cost = \
    #     prj_drawpath_II_temp(tgt_item, drawpath_ms, sample_num=SAMPLE_NUM, toggledebug=False)
    # pos_nrml_list, error_list_cm, time_cost = prj_drawpath_II(tgt_item, drawpath_ms, toggledebug=False)
    ...
    #     prj_drawpath(tgt_item, drawpath_ms, mode='DI', step=1, error_method=error_method, toggledebug=False)
    ...
    #     prj_drawpath(tgt_item, drawpath_ms, mode='QI', step=1, error_method=error_method, toggledebug=False)
    # print('metrology method time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 0, 1), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_QI.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    #
    # pos_nrml_list, _, time_cost = \
    #     prj_drawpath(tgt_item, drawpath_ms, mode='gaussian', step=1, error_method=error_method, toggledebug=False)
    ...
    #                  toggledebug=False)
    # print('B-spline time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(1, 0, 0), show_nrmls=False, transparency=1)
    # dump_mapping_res(f'{dump_f_name}_bs.pkl', tgt_item, drawpath_ms, pos_nrml_list, time_cost)
    # base.run()

    # pos_nrml_list, _, time_cost = \
    #     prj_drawpath(tgt_item, drawpath_ms, mode='EI', step=1, error_method=error_method, toggledebug=False)
    # print('metrology method time cost', time_cost)
    ...
    #
    # pos_nrml_list, _, time_cost = \
    #     prj_drawpath(tgt_item, drawpath_ms, mode='quad', step=1, error_method=error_method, toggledebug=False)
    ...
    #
    # pos_nrml_list, _, time_cost = \
    #     prj_drawpath(tgt_item, drawpath_ms, mode='rbf', step=1, error_method=error_method, toggledebug=False)
    ...
    # KERNEL = 'linear'
    # pos_nrml_list, _, time_cost = \
    #     prj_drawpath(tgt_item, drawpath_ms, mode='rbf_g', step=1, error_method=error_method, toggledebug=False)
    # print('rbf time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(1, 0, 0), show_nrmls=False, transparency=1)
    ...
    #
    # pos_nrml_list, _, time_cost = \
    #     prj_drawpath(tgt_item, drawpath_ms, mode='bp', step=1, error_method=error_method,
    #                            toggledebug=False)
    # print('rbf time cost', time_cost)
    # show_drawpath(flatten_nested_list(pos_nrml_list), color=(0, 0, 1), show_nrmls=False, transparency=1)
    ...
    # # show DRAW
    # drawpath_ms_D = resize_drawpath(du.load_drawpath('D.pkl'), 20, 20, space=0)
    # drawpath_ms_R = resize_drawpath(du.load_drawpath('R.pkl'), 20, 20, space=0)
    # drawpath_ms_A = resize_drawpath(du.load_drawpath('A.pkl'), 20, 20, space=0)
    # drawpath_ms_W = resize_drawpath(du.load_drawpath('W.pkl'), 20, 20, space=0)
    # # +40, +15, +2, -13
    # tgt_item.set_drawcenter((-10, 40, 0))
    #
    # pos_nrml_list, _ = prj_drawpath(tgt_item, drawpath_ms_D, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # tgt_item.set_drawcenter((0, -20, 0))
    # pos_nrml_list, _ = prj_drawpath(tgt_item, drawpath_ms_R, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # tgt_item.set_drawcenter((0, -13, 0))
    #
    # pos_nrml_list, _ = prj_drawpath(tgt_item, drawpath_ms_A, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # tgt_item.set_drawcenter((5, -15, 0))
    # pos_nrml_list, _ = prj_drawpath(tgt_item, drawpath_ms_W, use_pca=True)
    # show_drawpath(flatten_nested_list(pos_nrml_list))
    #
    # base.run()
