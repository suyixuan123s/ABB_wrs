...
def show_multi_ik(obj_rotmat, obj_pos, grasp):
    hndfa = rtqhe.RobotiqHE(enable_cc=True)
    obj_fixture = object.copy()
    obj_fixture.set_rotmat(obj_rotmat)
    obj_fixture.set_pos(obj_pos)
    obj_fixture.attach_to(base)
    # for i, item in enumerate(grasp_info_list):
    jaw_width, gl_jaw_center_pos, gl_jaw_center_rotmat, hnd_pos, hnd_rotmat = grasp
    hnd_rotmat = obj_rotmat.dot(gl_jaw_center_rotmat)
    hnd_pos = obj_rotmat.dot(gl_jaw_center_pos) + obj_pos
    ...
    hndfa.fix_to(pos=hnd_pos,
                 rotmat=hnd_rotmat, jawwidth=.04)
    for _ in range(100):
        seed = np.asarray([random.uniform(-np.pi, np.pi), random.uniform(-np.pi, np.pi), random.uniform(-np.pi, np.pi),
        ...
        if jnt_values is not None:
            ...
            robot.fk(component_name=component_name, jnt_values=jnt_values)
            if hndfa.is_mesh_collided(slopeforcd_high, toggle_debug=False):
                robot.gen_meshmodel(toggle_tcpcs=False).attach_to(base)
                # pass
            else:
                robot.gen_meshmodel(toggle_tcpcs=False).attach_to(base)
                print(f"IK Done!, feasible grasp ID {i}")
...
if __name__ == '__main__':
    ...
    object_start_rotmat = rm.rotmat_from_axangle((0, 0, 1), np.radians(180)).dot(
        rm.rotmat_from_axangle((1, 0, 0), np.radians(-90)).dot(rm.rotmat_from_axangle((0, 0, 1), np.radians(180))))
    ...
    show_multi_ik(object_fixture_homomat[:3, :3], object_fixture_pos, grasp_info_list[111])
    ...
    object_goal.set_rotmat(object_goal_rotmat)
    # object_goal.attach_to(base)
    show_ikfeasible_poses(object_goal_homomat[:3, :3], object_start_pos)
    base.run()
