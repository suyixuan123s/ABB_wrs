...
def facetboundary(objtrimesh, facet, facetnormal):
    ...
    if np.allclose(np.array(facetnormal), np.array([0, 0, 1]), atol=1e-4) or np.allclose(np.array(facetnormal),
                                                                                         np.array([0, 0, -1]),
                                                                                         atol=1e-4):
        facetmat4 = np.eye(4)
    else:
...
# #  Version 2
def collisionFace(bulletworld, faceContainer_assembled=None, faceContainer_mated=None, debugtoggle=False,
                  returnconvex=False, showthecontact=True):  # faceArray1 is collided, faceArray2 is origin
    collisionlist_mated = []
    ...
    for i in range(0, len(faceContainer_mated.facenplist)):
        ...
        for j in range(0, len(faceContainer_assembled.facenplist)):
            ...
            if faceContainer_assembled.facenplist[j].name in collisionTempAssembled:
                if faceContainer_mated.facenplist[i].name in collisionTempAssembled[
                    faceContainer_assembled.facenplist[j].name]:
                    result = collisionTempAssembled[faceContainer_assembled.facenplist[j].name][
                        faceContainer_mated.facenplist[i].name]

            elif faceContainer_mated.facenplist[i].name in collisionTempMated:
                if faceContainer_assembled.facenplist[j].name in collisionTempMated[
                    faceContainer_mated.facenplist[i].name]:
                    result = collisionTempMated[faceContainer_mated.facenplist[i].name][
                        faceContainer_assembled.facenplist[j].name]
            if result is None:
                result = collisiondetect(bulletworld, faceContainer_assembled.facenplist[j],
                                         faceContainer_mated.facenplist[i])  # Face collision Detection
                if faceContainer_assembled.facenplist[j].name in collisionTempAssembled:
                    collisionTempAssembled[faceContainer_assembled.facenplist[j].name][
                        faceContainer_mated.facenplist[i].name] = result.getNumContacts()
                else:
                    collisionTempAssembled[faceContainer_assembled.facenplist[j].name] = {
                        faceContainer_mated.facenplist[i].name: result.getNumContacts()}
            ...
            if result:
                ...
                if -1.1 < np.dot(facenormal_mated, facenormal_assembled) < -0.9:
                    boundaryface_assembled, Z2, InvMat2 = boundarydetect(
                        objtrimesh=faceContainer_assembled.facemeshlist[j],
                    ...
                    if debugtoggle:
                        ...
                        plt.show()
                    if (boundaryface_mated.intersection(
                            boundaryface_assembled).area > 10 and boundaryface_mated.intersects(
                            boundaryface_assembled)):
                        ...
                        convexhull_normal.append([drawTheArea(polygon1=boundaryface_mated,
                                                              polygon2=boundaryface_assembled, Z=Z2, invMat=InvMat2,
                                                              showthecontact=showthecontact), facenormal_mated])
...
def scoreStabilityRefToContactNormals(constraints, show=False):
    ...
    if len(uniqconstraints) == 1:  # case 1
        ...
        if numberOfFacets == 1:
            ...
            result = [np.rad2deg(
                rm.angle_between_vectors(sidevector, vector) * checkSymbol(sidevector, vector, normalofvector)) for
                      vector in searchspace]
            # print(result)
...
def plotFrictionCone(frictionConeVector, position):
    for i in frictionConeVector:
        base.pggen.plotArrow(base.render, spos=position, epos=position + i, length=15, thickness=1,
        ...
        for force in frictioncone_list[idx]:
            ...
            if show:
                base.pggen.plotArrow(base.render, spos=contact_pos,
                                     epos=np.array(contact_pos) + np.array(tmparray[-1][-3:]),
...
def torquespace(frictioncone_list, frictioncone_pos_list, objcom):
    ...
    for idx, contact_pos in enumerate(frictioncone_pos_list):
        ...
        for force in frictioncone_list[idx]:
            ...
            base.pggen.plotArrow(base.render, spos=contact_pos, epos=np.array(contact_pos) + np.array(tmparray[-1]),
                                 length=15, thickness=1,
