...
class StableChecher():
    ...
    def stabilityindex(self, objlist, sptPnt, center, collisionlist, normallist, facetcenterlist, u=0.3,
        ...
        surfaceequationlist = [np.around(humath.getsurfaceequation(normallist[i], facetcenterlist[i]), decimals=4) for i
                               in range(3)]
    ...
    def getcontactinfo(self, center, intersectionvertices, sptPnt, sptrate=1, polygon=True, point=False, ID=0,
                       name="none", thisdir="none",
                       show=True):
        ID = ID
                    ...
                    if type(surface) == list:
                        for k, patch in enumerate(surface):
                            for j, p in enumerate(patch):
                                pointsforcv[i][k][j] = p * np.array([sptrate, -sptrate]) + np.array(
                                    [size / 2, size / 2])
        ...
        cv2.circle(img_gray, (int(center[0] * sptrate + (size / 2)), int((-center[1]) * sptrate + (size / 2))),
                   radius=4,
    ...
    def checkStabilityFdistributed(self, convexandnormal, CoM, Fdistributed, u=0.3):
        ...
        for idx, points in enumerate(convexpoints):
            ...
            for idx2, point in enumerate(points):
                totalfrictioncone.append(
                    au.genFrictionCone(position=point, u=u, normal=normal, Fdistributed=Fdistributed[idx][idx2],
                                       show=False))
                coneposition.append(point)
        com = CoM[:3]
        totalfrictioncone = au.wrenchspace(frictioncone_list=totalfrictioncone,
