if __name__ == '__main__':
    ...
    for objn in objns:
        ...
        for i in range(10):
            hand = hndfa.genHand()
            freegriptst = fg.Freegrip(objpath, hand, faceangle=.9, segangle=.9, refine1min=5, togglebcdcdebug=True,
                                      useoverlap=True)
            ...
            for j, freegriprotmat in enumerate(freegriptst.griprotmats_planned):
                ...
                difflist.append(
                    (rm.degree_betweenvector(-hitnrml0, ctn0) + rm.degree_betweenvector(-hitnrml1, ctn1)) / 2)
                hand = hndfa.genHand()
