...
class stableholdplanner(fh.FreeholdContactpairs, slope.Slope):
    ...
    def checkstablity(self, lpsolve, friction=True):
        ...
        for i, objPnt in enumerate(self.pFaces):
            if lpsolve == False:
                ...
                picnodepath.append(
                    checker.getcontactinfo(center=com[i], intersectionvertices=intersectionvertices, sptPnt=sptPnt,
                                           sptrate=sptrate, polygon=True,
    ...
    def showRPYdistance(self):
        ...
        plt.tick_params(axis='both', direction='in', length=6, width=2, labelsize=30, pad=15)
        ax.set_xticks(
            np.arange(0, len(self.onebasedrotRPYdistance), int(math.ceil(len(self.onebasedrotRPYdistance) / 10))))
        ax.set_yticks(np.arange(0, 120, 20))
        plt.xlabel("placment No.", labelpad=5)
        plt.ylabel("RPY distance", labelpad=5)
        ax.tick_params(pad=3)
    ...
    def showfeatures(self, node, normal, placementfacetpairscenter, placementVertices, placementCoM, showCoM,
                     showNormal, showVertices):
        np.random.seed(0)
            ...
            for i in range(len(normal)):
                gm.gen_arrow(spos=np.array([placementfacetpairscenter[i][0],
                             ...
                             epos=np.array([placementfacetpairscenter[i][0] + length * normal[i][0],
                                            ...
                                            placementfacetpairscenter[i][2] + length * normal[i][2]]), thickness=0.005,
                             rgba=color).attach_to(node)
        if showCoM == True:
            gm.gen_sphere(pos=(placementCoM[0], placementCoM[1], placementCoM[2]), radius=.003, rgba=color).attach_to(
                node)
        ...
        if showVertices == True:
            for vertices in placementVertices:
                ...
                for i in range(len(vertices)):
                    gm.gen_sphere(pos=(vertices[i][0], vertices[i][1], vertices[i][2]), radius=.005,
