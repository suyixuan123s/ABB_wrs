...
if __name__ == "__main__":
    ...
    rpy_error_print = printonscreen(pos=(-1, 0.6, 0),
                                    words="Error range on rpy: " + str(np.degrees(errorrange_rpy)) + " (degree)")
    ...
    def update(objbm, comlist, count, countforrot, time, objposnode, error, estimation, estimation_com, clock, task):
        ...
        if time[0] == 10:
            objbm[0].set_mass(1)
            # base.attach_internal_update_obj(objbm[0])
            objposnode[0] = printonscreen(pos=(-1, 0.4, 0), words="Initial ObjPos (Real): x=" + str(
                0.600 + np.round(error[0][0], decimals=5)) + "  " + "y=" + str(
                0 + np.round(error[0][1], decimals=5)) + "  " + "z=" + str(
                heightlist[count[0]] + np.round(error[0][1], decimals=5)))
            objposnode[1] = printonscreen(pos=(-1, -0.2, 0),
                                          words="Repetition: " + str(count[1] + 1) + "/" + str(repetition))
            objposnode[4] = printonscreen(pos=(-1, 0.3, 0),
                                          words="Initial ObjPos (Ideal): x=" + str(0.600) + "  " + "y=" + str(
                                              0) + "  " + "z=" + str(heightlist[count[0]]))
            objposnode[5] = printonscreen(pos=(-1, 0.2, 0),
                                          words="Rot error: x axis: " + str(
                                              np.round(error[0][3], decimals=5)) + "  " + "y axis: " + str(
                                              np.round(error[0][4], decimals=2)) + "  " + "z axis: " + str(
                                              np.rad2deg(np.round(error[0][5], decimals=5))) + "  (degree)")
        ...
        if time[0] == 320:
            ...
            gm.gen_sphere(pos=np.round(
                np.dot(objbm[0].get_homomat(), np.concatenate((objbm[0].cm.objtrm.center_mass, np.array([1])), axis=0)),
                decimals=4)[:3],
        ...
        if time[0] == 400:
            ...
            if objcom[0] <= com[0] + 0.0001 and objcom[0] >= com[0] - 0.0001 \
                ...
                criteria = 0
            estimation.append(
                [countforrot[0], RotMatnozeroID[countforrot[0]], error[0], [.600, 0, heightlist[count[0]]], objcom,
                 criteria])
    ...
    taskMgr.add(update,
                extraArgs=[objbm, comlist, count, countforrot, time, objposnode, error, estimation, estimation_com,
                           clock], appendTask=True)
    base.run()
